...
function UsefulBadge({ useful }: { useful: boolean | null }) {
  if (useful === true) {
    return (
      <span
  ...
  return (
    <span className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium text-muted-foreground border border-muted">
      미판단
    </span>
  );
}
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-64">요약</TableHead>
            <TableHead className="w-20 text-center">유용성</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {isLoading ? (
